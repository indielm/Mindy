...
void drawDebugText() {
  ...
  debugText("" +(benchmark.get(1)-benchmark.get(0)));
  debugText("" +(benchmark.get(2)-benchmark.get(1)));
  debugText("renderCam: " +(benchmark.get(3)-benchmark.get(2)));
  debugText("" +(benchmark.get(4)-benchmark.get(3)));
  //debugText("" +(benchmark.get(5)-benchmark.get(4)));
}
...
void renderDbgLayer() {
  ...
  //println(cursorXY[0]*tileSize, cursorXY[1]*tileSize);
  //pgDbg.fill(255);
  pgDbg.stroke(80);
  pgDbg.translate(0,0,0);
  //pgDbg.rect(1024-8,1024-0,24,24);
  pgDbg.endDraw();
}
